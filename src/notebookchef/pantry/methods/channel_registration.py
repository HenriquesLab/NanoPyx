...
def on_button_register(b):
    ...
    dataset_registered = channel_registration.estimate_channel_registration(
        dataset_original,
        ref_channel,
        max_shift,
        n_blocks,
    ...
    gui_reg._main_display.children = gui_reg._main_display.children + (
        stackview.slice(
            dataset_registered,
            colormap=gui_reg["cmaps"].value,
...
gui_reg.add_int_slider(
    "blocks", description="Blocks per axis", min=1, max=10, value=5
)
gui_reg.add_float_slider(
    "min_sim",
...
gui_reg.add_callback("register", on_button_register, {}, description="Register")
gui_reg.show()
