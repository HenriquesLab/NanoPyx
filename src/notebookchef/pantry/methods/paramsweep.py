...
def run_param_sweep(b):
    ...
    sens_idx, rad_idx = np.unravel_index(
        np.argmax(param_sweep_out), param_sweep_out.shape
    )
    ...
    print(
        f"Optimal sensitivity is: {optimal_sensitivity}; and optimal radius is: {optimal_radius}"
    )


gui_param_sweep.add_int_slider(
    "magnification", description="Magnification:", min=1, max=10, value=2
)
gui_param_sweep.add_text(
    "sensitivities",
    description="List of sensitivities (comma-separated):",
    value="1, 2",
)
gui_param_sweep.add_text(
    "raddi", description="List of radii (comma-separated):", value="1, 1.5"
)
gui_param_sweep.add_dropdown(
    "temp_corr",
    description="Temperature Correction:",
    options=["AVG", "VAR", "TAC2"],
    value="AVG",
)
gui_param_sweep.add_checkbox(
    "use_decorr", description="Use Decorrelation instead of FRC", value=False
)
gui_param_sweep.add_dropdown(
    "cmaps",
...
gui_param_sweep.add_callback("run", run_param_sweep, {}, description="Run")
gui_param_sweep.show()
