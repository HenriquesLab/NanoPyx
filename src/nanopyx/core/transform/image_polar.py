...
@timeit2
def catmull_rom_polar(image: np.ndarray):
    interpolator = catmull_rom.Interpolator(image)
    return interpolator.polar()


@timeit2
def lanczos_polar(image: np.ndarray, taps: int = 3):
    interpolator = lanczos.Interpolator(image, taps)
    return interpolator.polar()


@timeit2
def bicubic_polar(image: np.ndarray):
    interpolator = bicubic.Interpolator(image)
    return interpolator.polar()


@timeit2
def bilinear_polar(image: np.ndarray):
    interpolator = bilinear.Interpolator(image)
    return interpolator.polar()


@timeit2
def nearest_neighbor_polar(image: np.ndarray):
    interpolator = nearest_neighbor.Interpolator(image)
    return interpolator.polar()


@timeit2
def skimage_polar(image: np.ndarray):
    return warp_polar(image)
