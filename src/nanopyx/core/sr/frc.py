...
class FIRECalculator(object):
    """Calculate FRC resolution

    Inspired by: https://colab.research.google.com/drive/1svyAqyjpdo_hIG8FSCjAmhNznqDq2sFm?usp=sharing#scrollTo=R2rCXeIGF6zV
    """
    
    def __init__(self, pixel_size=1):
        self.pixel_size = pixel_size
        self.FRC_values = None
        self.spat_freq = None
        self.FIRE_number = None
        
    def create_distance_map(self, img):
    ...
    def get_intersections(self, frc_curve, threshold_curve):
    ...
    def calculate_fire_number(self, img_1, img_2):
        ...
        fft_1, fft_2 = np.fft.fft2(img_1), np.fft.fft2(img_2)
        
        af1f2 = np.fft.fftshift(np.real(fft_1 * np.conj(fft_2)))
        
        af1_2, af2_2 = np.fft.fftshift(np.abs(fft_1)**2), np.fft.fftshift(np.abs(fft_2)**2)
        
        distancemap = self.create_distance_map(img_1)
        ...
        for d in range(1,max_dist):
            ...
            FRC_values[d] = (f1f2_r[d]/np.sqrt(f12_r[d]*f22_r[d]))
        
        FRC_values = pd.DataFrame(FRC_values, columns=['FRC'])
        FRC_values.interpolate(inplace=True, method='linear')
        FRC_values = FRC_values.to_numpy()
        self.FRC_values = FRC_values
        spat_freq = np.linspace(0, 1/(2*self.pixel_size), len(FRC_values))
        self.spat_freq = spat_freq
        
        frc_curve = np.concatenate((spat_freq.reshape((spat_freq.shape[0], 1)), FRC_values.reshape(FRC_values.shape[0], 1)), axis=1)

        threshold_curve = np.empty((spat_freq.shape[0]))
        threshold_curve.fill(1/7)
        intersection = self.get_intersections(frc_curve, threshold_curve)

        if len(intersection) > 0:
            FRCresolution = 1/intersection[0]
            
        else:
            FRCresolution = 0

        self.FIRE_number = FRCresolution
        return FRCresolution
        
    def plot_frc_curve(self):
        plt.plot(self.spat_freq, self.FRC_values)
        plt.axhline(1/7, color='r', linestyle='-')
        plt.xlabel('Spatial frequency [1/nm]')
        plt.ylabel('FRC')
        plt.title(f"FRC resolution: {np.round(self.FIRE_number, 1)} nm");
        plt.show()

# class FIRECalculator_old(object):
...
#             lower_left_v = image[lower_left_i]
#             lower_right_v = image[lower_right_i]
#             upper_right_v = image[upper_right_i]
#             upper_left_v = image[upper_left_i]
...
#                 values = self.get_interpolated_value(x, y, size)
#                 sum0 += values[0]
#                 sum1 += values[1]
#                 sum2 += values[2]
#                 num_sum += 1
                
#             results[radius] = [radius, sum0/math.sqrt(sum1*sum2), num_sum]
