...
cdef class DecorrAnalysis:
    ...
    cdef float[:, :] _compute_d(self):
        ...
        for refine in range(2):
            for k in range(self.n_g):
                ...
                fft = np.fft.fftshift(np.fft.fft2(blurred)).astype(np.complex128)
    ...
    cdef _run_analysis(self):
        ...
        img_fft = np.fft.fftshift(np.fft.fft2(temp)).astype(np.complex128)
