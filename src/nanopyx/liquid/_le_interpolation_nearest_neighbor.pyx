...
from ._le_interpolation_nearest_neighbor_ import shift_magnify_rotate as _py_shift_magnify_rotate
from ._le_interpolation_nearest_neighbor_ import njit_shift_magnify_rotate as _njit_shift_magnify_rotate
...
class ShiftMagnifyRotate(LiquidEngine):
    """
    Shift, Magnify and Rotate (similarity transform) using the NanoPyx Liquid Engine
    """
    ...
    def run(self, image, shift_row, shift_col, float magnification_row, float magnification_col, float angle) -> np.ndarray:
        """
        Shift and magnify an image using nearest neighbor interpolation
        :param image: The image to shift and magnify
        :type image: np.ndarray
        ...
        """
        image, shift_row, shift_col = self._parse_arguments(image, shift_row, shift_col)
        return self._run(image, shift_row, shift_col, magnification_row, magnification_col, angle)

    def benchmark(self, image, shift_row, shift_col, float magnification_row, float magnification_col, float angle):
        """
        Benchmark the ShiftAndMagnify run function in multiple run types
        :param image: The image to shift and magnify
        :type image: np.ndarray
        ...
        """
        image, shift_row, shift_col = self._parse_arguments(image, shift_row, shift_col)
        return super().benchmark(image, shift_row, shift_col, magnification_row, magnification_col, angle)

    def _run_opencl(self, image, shift_row, shift_col, float magnification_row, float magnification_col, float angle) -> np.ndarray:
        code = self._get_cl_code("_le_interpolation_nearest_neighbor_.cl")
    ...
    def _run_python(self, image, shift_row, shift_col, magnification_row, magnification_col) -> np.ndarray:
        image_out = _py_shift_magnify_rotate(image, shift_row, shift_col, magnification_row, magnification_col)
        return image_out
    ...
    def _run_njit(
        ...
        shift_col=np.zeros((1,),dtype=np.float32), 
        magnification_row=1, magnification_col=1) -> np.ndarray:
        image_out = _njit_shift_magnify_rotate(image, shift_row, shift_col, magnification_row, magnification_col)
        return image_out
