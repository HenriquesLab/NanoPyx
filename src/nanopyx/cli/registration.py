...
def linkinpy_estimate_channel_registration(
    input_image: str,
    output_image: str,
    output_path_translation_masks: str,
    ref_channel: int,
    max_shift: float,
    blocks_per_axis: int,
    ...
    output = estimate_channel_registration(
        ...
        translation_mask_save_path=output_path_translation_masks,
        apply=apply,
    )
...
def linkinpy_estimate_channel_registration_main() -> None:
    ...
    parser.add_argument("output_path_translation_masks", help="Output path for translation masks.")
    ...
    linkinpy_estimate_channel_registration(
        ...
        output_path_translation_masks=args.output_path_translation_masks,
...
def linkinpy_apply_channel_registration(
    input_image: str,
    output_image: str,
    input_image_translation_masks: str,
) -> None:
    image = imread(input_image)
    translation_masks = imread(input_image_translation_masks)
...
def linkinpy_apply_channel_registration_main() -> None:
    ...
    parser.add_argument("input_image", help="Input multi-channel image path.")
    parser.add_argument("output_image", help="Output aligned image path.")
    parser.add_argument("input_image_translation_masks", help="Input translation masks image path.")
    args = parser.parse_args()
    linkinpy_apply_channel_registration(
        input_image=args.input_image,
        output_image=args.output_image,
        input_image_translation_masks=args.input_image_translation_masks,
    )
...
def linkinpy_estimate_drift_alignment(
    ...
    output = estimate_drift_alignment(
        ...
        save_drift_table_path=output_table_drift,
...
def linkinpy_estimate_drift_alignment_main() -> None:
    ...
    parser.add_argument("output_table_drift", help="Output drift table path.")
    parser.add_argument("--save-as-npy", "--save_as_npy", dest="save_as_npy", action=argparse.BooleanOptionalAction, default=False)
    ...
    linkinpy_estimate_drift_alignment(
        input_image=args.input_image,
        output_image=args.output_image,
        output_table_drift=args.output_table_drift,
...
def linkinpy_apply_drift_alignment(
    input_image: str,
    output_image: str,
    input_table_drift: str,
) -> None:
    image = imread(input_image)
    output = apply_drift_alignment(image_array=image, path=input_table_drift)
    imwrite(output_image, output)
...
def linkinpy_apply_drift_alignment_main() -> None:
    ...
    parser.add_argument("input_image", help="Input timelapse image path.")
    parser.add_argument("output_image", help="Output drift-corrected image path.")
    parser.add_argument("input_table_drift", help="Input drift table path.")
    args = parser.parse_args()
    linkinpy_apply_drift_alignment(
        input_image=args.input_image,
        output_image=args.output_image,
        input_table_drift=args.input_table_drift,
    )
...
def linkinpy_Registration_EstimateChannelRegistration(
    ...
    linkinpy_estimate_channel_registration(
        ...
        output_path_translation_masks=output_path_translation_masks,
        ref_channel=ref_channel,
        max_shift=max_shift,
        blocks_per_axis=blocks_per_axis,
        min_similarity=min_similarity,
...
def linkinpy_Registration_ApplyChannelRegistration(
    ...
    linkinpy_apply_channel_registration(
        input_image=input_image,
        output_image=output_image,
        input_image_translation_masks=input_image_translation_masks,
    )
...
def linkinpy_Registration_EstimateDriftAlignment(
    ...
    linkinpy_estimate_drift_alignment(
        input_image=input_image,
        output_image=output_image,
        output_table_drift=output_table_drift,
        save_as_npy=save_as_npy,
        time_averaging=time_averaging,
        max_expected_drift=max_expected_drift,
        ref_option=ref_option,
...
def linkinpy_Registration_ApplyDriftAlignment(
    ...
    linkinpy_apply_drift_alignment(
        input_image=input_image,
        output_image=output_image,
        input_table_drift=input_table_drift,
    )
