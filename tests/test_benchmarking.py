...
from nanopyx.core.generate.beads import (
    generate_timelapse_drift,
    generate_channel_misalignment,
)


def test_benchmark_interpolation_bicubic():
    random_image_with_ramp = get_ramp(64, 64)
    bicubic_sm = (
...
def test_benchmark_interpolation_catmull():
    random_image_with_ramp = get_ramp(64, 64)
    cr_sm = (
...
def test_benchmark_interpolation_lanczos():
    random_image_with_ramp = get_ramp(64, 64)
    l_sm = (
        nanopyx.core.transform._le_interpolation_catmull_rom.ShiftAndMagnify()
    )
...
def test_benchmark_interpolation_nn():
    random_image_with_ramp = get_ramp(64, 64)
    nn_sm = (
    ...
    nn_pt.benchmark(
        random_image_with_ramp,
        (random_image_with_ramp.shape[0], random_image_with_ramp.shape[1]),
        "log",
    )
...
def test_benchmark_conv2d():
    ...
    conv2d.benchmark(
        random_image_with_ramp, np.ones((3, 3)).astype(np.float32)
    )


def test_benchmark_esrrf():
    random_image_with_ramp = get_ramp(64, 64)
...
def test_benchmark_nlm():
    random_image_with_ramp = get_ramp(64, 64)
    nlm = nanopyx.core.transform._le_nlm_denoising.NLMDenoising()
    nlm.benchmark(random_image_with_ramp)


def test_benchmark_channel_reg():
    random_channel_misalignment = generate_channel_misalignment()
    channel_reg = (
...
def test_benchmark_drift_correction():
